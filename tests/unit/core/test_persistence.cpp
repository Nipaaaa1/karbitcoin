...
TEST_F(PersistenceTest, MetadataPersistence) {
  ...
  {
    Blockchain bc_reloaded(
        1, data_dir); // Constructor diff 1 should be overridden by saved 3
...
TEST_F(PersistenceTest, UtxoRecoveryFromBlocks) {
  std::string miner = "miner_addr";
  {
    Blockchain bc(1, data_dir);
    bc.minePendingTransactions(miner);
  ...
  fs::remove(data_dir + "/utxo.json");
  EXPECT_FALSE(fs::exists(data_dir + "/utxo.json"));
...
TEST_F(PersistenceTest, CorruptBlockFileHandling) {
  {
    Blockchain bc(1, data_dir);
    bc.minePendingTransactions("miner");
  }

  std::ofstream ofs(data_dir + "/blocks/block_1.json");
  ofs << "THIS IS NOT JSON {[[";
  ofs.close();

  {
    Blockchain bc_reloaded(1, data_dir);
    EXPECT_EQ(bc_reloaded.getHeight(), 1);
  }
}
